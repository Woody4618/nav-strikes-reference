...
description:
  ...
  using SPL Token 2022 extensions, enabling institutional-grade subscription
  and redemption with atomic settlement."
tags:
  - token extensions
  - token 2022
  - ts
...
A fund is a pool of assets that many investors own together.
**NAV (Net Asset Value)** is simply the price of one share in a fund. Think of
it like this:
...
│   ✅ No front-running (execute at exact NAV)                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
You can use the [source code](https://github.com/example/nav-strikes-reference)
of this implementation to try NAV strikes locally.
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. Fund Token Creation (Token-2022 + Metadata)             │
│     └─> Default State: FROZEN                               │
│     └─> Freeze Authority: Fund Administrator                │
│     └─> Metadata: NAV, Strike Schedule, AUM                 │
│                                                             │
│  2. Whitelist Investors (KYC/AML approved)                  │
│     └─> Create token account                                │
│     └─> Thaw account for trading                            │
│                                                             │
│  3. Order Submission (before strike)                        │
│     ├─> Investor delegates USDC (for subscription)          │
│     └─> Investor delegates shares (for redemption)          │
│                                                             │
│  4. NAV Strike Execution                                    │
│     ├─> Update NAV on-chain                                 │
│     ├─> Process subscriptions: USDC → Shares                │
│     └─> Process redemptions: Shares → USDC                  │
│         (All atomic - complete or fail together)            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
export class NAVStrikeEngine {
  ...
  async createFundToken(
    ...
    console.log(`✅ Fund token created: ${mintKeypair.publicKey.toBase58()}`);
  ...
  async delegateUSDCForSubscription(
    ...
    console.log(`✅ Delegated $${amount} USDC for subscription`);
  }
  ...
  async delegateSharesForRedemption(
    ...
    console.log(`✅ Delegated ${shareAmount} shares for redemption`);
  }
  ...
  queueOrder(
    ...
    console.log(`📝 Order queued: ${order.orderId} - ${orderType} ${amount}`);
  ...
  async processSubscription(params: SubscriptionParams): Promise<string> {
    ...
    console.log(
      `✅ Subscription: $${usdcAmount} → ${sharesToMint.toFixed(2)} shares`
    );
  ...
  async processRedemption(params: RedemptionParams): Promise<string> {
    ...
    console.log(
      `✅ Redemption: ${shareAmount} shares → $${usdcToPay.toFixed(2)}`
    );
  ...
  async executeStrike(
    ...
    console.log(`\n⚡ Executing NAV Strike at $${newNAV.toFixed(6)}`);
    ...
    for (const order of subscriptions) {
      try {
        ...
        console.error(`   ❌ Order ${order.orderId} failed`);
        order.status = "failed";
      }
    }
    ...
    console.log(`✅ Strike complete. AUM: $${this.totalAUM.toLocaleString()}`);
  }
}
```
...
async function runNAVStrikesExample() {
  ...
  console.log("🚀 Starting NAV Strikes Demo\n");
  ...
  for (const [name, investor, amount] of [
    ...
    console.log(`💵 Minted $${amount} USDC to ${name}`);
  }
...
## Subscription Flow (USDC → Shares)

The subscription process converts investor USDC into fund shares atomically:
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   Instruction 1: Transfer USDC                              │
│   ┌──────────────┐                    ┌──────────────┐      │
│   │  Investor    │  ──── $100 ────→   │    Fund      │      │
│   │  USDC Acct   │     (delegated)    │  USDC Acct   │      │
│   └──────────────┘                    └──────────────┘      │
│                                                             │
│   Instruction 2: Mint Shares                                │
│   ┌──────────────┐                    ┌──────────────┐      │
│   │  Fund Mint   │  ── 100 shares ──→ │  Investor    │      │
│   │  Authority   │    (at NAV $1)     │  Share Acct  │      │
│   └──────────────┘                    └──────────────┘      │
│                                                             │
│   ✅ Both or neither - atomic execution                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

## Redemption Flow (Shares → USDC)

The redemption process burns shares and pays USDC atomically:
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   Instruction 1: Burn Shares                                │
│   ┌──────────────┐                    ┌──────────────┐      │
│   │  Investor    │  ── 50 shares ──→  │    BURN      │      │
│   │  Share Acct  │    (delegated)     │              │      │
│   └──────────────┘                    └──────────────┘      │
│                                                             │
│   Instruction 2: Transfer USDC                              │
│   ┌──────────────┐                    ┌──────────────┐      │
│   │    Fund      │  ── $51.00 ────→   │  Investor    │      │
│   │  USDC Acct   │   (50 × $1.02)     │  USDC Acct   │      │
│   └──────────────┘                    └──────────────┘      │
│                                                             │
│   ✅ Both or neither - atomic execution                     │
│                                                             │
...
## Comparison: Traditional vs Solana NAV Strikes

| Aspect               | Traditional          | Solana NAV Strikes       |
| -------------------- | -------------------- | ------------------------ |
| Strikes per day      | 1                    | 4+ (configurable)        |
| Settlement time      | T+1 to T+2           | &lt;1 second             |
| Cost per transaction | $50-200              | &lt;$0.01                |
| NAV transparency     | End-of-day report    | On-chain, real-time      |
| Front-running risk   | High (stale NAV)     | Zero (atomic)            |
| Intermediaries       | Transfer agent, fund | None (direct settlement) |
| Investor access      | Business hours       | 24/7 order submission    |
| Compliance           | Manual KYC           | On-chain whitelist       |
